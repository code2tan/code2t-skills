...
| 文件 | 说明 |
|------|------|
| `content.xml` | 含 `<p>[图片占位 #N: 名称]</p>` 占位段落和内联表格的 XML 内容 |
...
图片处理走"占位符 → 末尾插入 → 定位 → 替换"的固定流程。**占位符不是可选的**：它是 agent
唯一能把"meta.json 里第 N 张图片"对应到"目标文档里第 N 个目标位置"的桥梁。源文档里的
`<img>` 在 prepare.py 的 XML 中已经被替换为：

```
<p>[图片占位 #N: image.png]</p>
```

其中 `N` 是 1-based 索引，与 `meta.json` 的 `downloaded_images[N-1]` 一一对应。occurrences
都被 `<p>` 包起来——这一步是必要的，因为飞书源文档中 `<img>` 通常是块级元素，如果占位是
裸文本，overwrite 解析时会被当成游离文本而丢弃，agent 之后就找不到任何占位 block，"图片
处理"就会退化成"全部追加在文档末尾"，丢失原文位置信息。

### 4a. 将图片插入文档末尾

对 `meta.json` 中 `downloaded_images` 数组的**每张图片按数组顺序**执行以下命令一次（每次只插一张）：
...
从返回中找出：
- 所有 `[图片占位 #N: ...]` 文本 block 的 ID（**通过文本中的 `#N` 唯一定位**——不要只匹配名字，因为飞书图片名往往全是默认的 "image.png"，单凭名字会撞车）
- 每个占位 block **前一个 block** 的 ID（用于第 4d 移动锚点）——这是图片在原文中的"插入位置"
- 每张追加在文档末尾的图片 block 的 ID（即 `block_type=27` 的 image block；按 fetch 返回中的出现顺序对应 `downloaded_images[0..N-1]`）

**关键校验**：如果 fetch 后**找不到任何 `[图片占位` 文本**，但 `meta.json.downloaded_images` 不为空，
说明 overwrite 时占位被丢失了——这是异常状态，**必须停下来报错告诉用户**，不要跳过删除步骤
也不要继续往下做。继续执行只会得到"所有图片堆在文档末尾"的错误结果。
排查方向：检查 `content.xml` 中是否真的存在 `<p>[图片占位` 字符串；如果存在，可能是 lark-cli
overwrite 行为发生了变化，需要查看 lark-cli 版本和 release notes。
...
> ⚠️ **删除前必须先记下每个占位 block 的"前一个 block ID"**（4b 中提到的"移动锚点"）。
> 删除占位本身不会影响其他 block 的 ID，但占位被删后，"占位的前一个 block" 就成了"图片该
> 移到它后面"的锚点，这个映射关系是接下来 4d 的输入。

### 4d. 将图片移动到正确位置

`block_move_after` 一次只能移动一组图片到一个锚点。**对每张图片单独执行一次**该命令，
按 4b 中建立的"图片 #N → 锚点 block_id"映射逐张移动：
...
1. 脚本自动提取 `<img>` 标签（兼容 `token="..."` 和 `src="..."` 两种格式）并替换为 `<p>[图片占位 #N: 名称]</p>`，N 是 1-based 索引，与 `meta.json` 的 `downloaded_images[N-1]` 一一对应
2. 用 `media-insert` 从本地文件上传（总是追加到末尾），按数组顺序逐张插入
3. fetch with-ids 后，通过占位文本中的 `#N` 唯一定位每个占位 block 和它前面的锚点 block
4. 先删除占位 block 再移动图片，避免 block_id 变化
5. 用 `block_move_after` 把图片移动到对应锚点之后（每张图片一次命令）

**为什么需要 `#N` 索引**：飞书图片在源文档里 name 多数是默认的 `image.png`，没有索引就完全无法区分。索引让每个占位字符串唯一，agent 通过 `fetch` 返回的文本内容做精确匹配即可定位 block。
