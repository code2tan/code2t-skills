...
整个过程分 5 个阶段，其中第一阶段由 Python 脚本自动完成（包括提取嵌入电子表格并转为内联表格）：
...
5. **嵌入电子表格转为静态内联表格**：源文档中通过 `<sheet>` 标签嵌入的飞书电子表格会被自动读取并转换为内联 HTML `<table>`。转换后保留数据内容和行列结构，但不保留原始电子表格的公式、样式和交互功能。如果某个电子表格因权限等问题无法读取，原 `<sheet>` 标签会被保留不变。
